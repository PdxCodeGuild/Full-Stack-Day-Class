...
def color_source(source_str):
    """Take source code as a string, guess what langauge it is, and return an
    HTML highilghted version of it.

    Returns the guessed langauge and HTML highlighted version as a
    ColoredSource instance.
    """
    lexer = pygments.lexers.guess_lexer(source_str)
    colored_source_html = pygments.highlight(
        source_str, lexer, HTML_FORMATTER)
